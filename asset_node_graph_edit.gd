...
func create_graph_from_parsed_data() -> void:
    ...
    scroll_offset = root_gn.position_offset
...
func new_graph_node(asset_node: HyAssetNode) -> CustomGraphNode:
    ...
    if asset_node.an_type == "<NO TYPE>":
        ...
        if asset_node.an_name == "<NO NAME>":
            graph_node.title = asset_node.an_type
        else:
            graph_node.title = "%s (%s)" % [asset_node.an_type, asset_node.an_name]
    ...
    if use_json_positions:
        ...
        print("Positioning node %s at %s" % [asset_node.an_node_id.substr(0, 14) + "...", graph_node.position_offset])
